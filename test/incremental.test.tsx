...
describe('it', () => {
  it('generates an id', () => {
    const wrapper = ({ children }: React.PropsWithChildren) => <IncrementalProvider>{children}</IncrementalProvider>;
  ...
  it('generates an id starting from a value', () => {
    const wrapper = ({ children }: React.PropsWithChildren) => <IncrementalProvider initialValue={10}>{children}</IncrementalProvider>;
    const { result } = renderHook(() => useId(), { wrapper });
  ...
  it('generates an id with a prefix', () => {
    const wrapper = ({ children }: React.PropsWithChildren) => <IncrementalProvider prefix="test-">{children}</IncrementalProvider>;
    const { result } = renderHook(() => useId(), { wrapper });
